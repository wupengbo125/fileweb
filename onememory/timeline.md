...
- 2026-09-20 [fw-20260920-a1] app.py：api_upload 换 email.parser 解析 multipart（cgi 在 py3.13 移除）；文件写与 api_sync 加全局 _LOCK 防并发；safe_path 路径越界防护已有（resolve+is_relative_to），未动；git 命令原样保留。已实测上传/写文件/路径穿越/同步均正常。
- 2026-09-20 19:00 [12046848243714] .gitignore 追加 GitHub 官方 Python 模板全量条目（__pycache__/、*.pyc、build/dist/、venv/、.env、pytest/mypy 缓存等），git rm -r --cached 移除历史误跟踪的 __pycache__/app.cpython-310.pyc；解决每小时同步脚本因未跟踪 pycache 把 fileweb 判为"本地有改动"跳过的问题
